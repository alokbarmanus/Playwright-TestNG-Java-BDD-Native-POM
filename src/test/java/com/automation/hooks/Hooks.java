...
/**
 * Cucumber Hooks — manages Playwright lifecycle and test-data loading for each scenario.
 *
 * <ul>
 *   <li>{@code @Before}  — resolves any {@code @dataFile:} tag, loads JSON test data into
 *       {@link ScenarioContext}, then initialises Playwright.</li>
 *   <li>{@code @After}   — captures a screenshot on failure, then tears everything down.</li>
 * </ul>
 *
 * <h3>@dataFile tag convention</h3>
 * <pre>&#64;dataFile:env/&lt;env&gt;/data/someData.json</pre>
 * The path prefix {@code env/} maps to the classpath folder {@code environments/}.
 * {@code ${env}} in the path is replaced with the active environment name from
 * {@link ConfigReader#getActiveEnv()}.
 */
public class Hooks {
     ...
     * Loads JSON test data (if {@code @dataFile:} tag present), then spins up Playwright.
     */
    @Before
     ...
     * Attaches a full-page screenshot if the scenario failed, then closes
            ...
            if (scenario.isFailed()) {
                ...
                if (screenshot != null) {
                    ExtentManager.getTest().fail("Scenario failed. Screenshot attached.")
                        .addScreenCaptureFromPath("../cucumber-reports/FAILURE_SCREENSHOT_" + scenario.getName() + ".png");
                } else {
    ...
    /**
     * Scans the scenario tags for one starting with {@code @dataFile:},
     * resolves the path, and loads the JSON data into {@link ScenarioContext}.
     *
     * <p>Path resolution rules:
     * <ul>
     *   <li>{@code ${env}} → replaced with the active environment name
     *       (e.g. {@code dev}, {@code sit}, {@code uat})</li>
     *   <li>Leading {@code env/} → replaced with {@code environments/} to
     *       match the classpath resource folder</li>
     * </ul>
     */
    private void loadJsonDataIfTagged(Scenario scenario) {
        scenario.getSourceTagNames().stream()
                ...
                .ifPresent(tag -> {
                    ...
                    String resolvedPath = rawPath.replace(
                            "${env}", ConfigReader.getActiveEnv());
